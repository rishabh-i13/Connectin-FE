...
const RecommendedUser = ({ user }) => {
	...
	switch (connectionStatus?.data?.status) {
		...
		default: // notConnected
			return (
				<button
					className='px-3 py-1 rounded-full text-sm border border-[#8E00F4] text-[#8E00F4] hover:bg-gradient-to-r from-[#360072] to-[#8E00F4] hover:text-white transition-colors duration-200 flex items-center'
					onClick={handleConnect}
				>
					<ImUserPlus size={16} className='mr-1' />
					Connect
				</button>
			);
	}
